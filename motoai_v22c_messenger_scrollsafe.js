/* motoai_v22c_messenger_scrolltags.js
   MotoAI v22c — Messenger-style • SmartCalc • UltraSafe • ScrollTags Suggestions
   - UI kiểu Messenger (80%) + thanh cuộn tags "🏍️ Xe số", "🛵 Xe ga",...
   - Giữ SmartCalc (có “xe giá rẻ”), delay 2.5–5s, auto-avoid footer/quick-call, dark/light
   - ScrollTags auto-hide khi gõ (an toàn cho iOS)
*/
(function(){
  // Ngăn chặn việc load lặp lại
  if (window.MotoAI_v22c_MESSENGER_SCROLLTAGS_LOADED) return;
  window.MotoAI_v22c_MESSENGER_SCROLLTAGS_LOADED = true;
  ...
  /* === Scroll Tags gợi ý (thay cho dropdown) === */
  #mta-sugs{
    padding: 8px 10px;
    background: #f7f9fc;
    ...
    max-height: 60px; /* Đủ cho 1 hàng */
    opacity: 1;
  }
  #mta-sugs::-webkit-scrollbar{display:none} /* Ẩn scrollbar (Chrome, Safari) */
  .sug-tag{
    display: inline-block;
    flex-shrink: 0; /* Không co thẻ */
    border: none;
    background: #fff;
    padding: 8px 12px;
    border-radius: 18px; /* Bo tròn giống tin nhắn */
    cursor: pointer;
    font-weight: 600;
    box-shadow: 0 1px 2px rgba(0,0,0,.08);
    border: 1px solid rgba(0,0,0,.06);
  }
  .sug-tag:hover{background:#eef4ff}
  /* === Hết phần Scroll Tags === */

  @media(max-width:520px){
    #mta-card{width:calc(100% - 16px);right:8px;left:8px;height:72vh}
    #mta-bubble{width:56px;height:56px}
    /* .sug-list{grid-template-columns:1fr} (đã xóa) */
  }
  @media(prefers-color-scheme:dark){
    ...
    /* Dark mode cho Scroll Tags */
    #mta-sugs{background:#1f2127;border-top:1px solid rgba(255,255,255,.08)}
    .sug-tag{background:#2a2d34;color:#eaeef3;border:1px solid rgba(255,255,255,.1)}
    .sug-tag:hover{background:#2d3441}
  }
  ...
  const K = {sess:'MotoAI_v22b_session', ctx:'MotoAI_v22b_ctx'}; // Giữ nguyên key để giữ lịch sử chat
  ...
  // ===== ScrollTags Suggestions (thay cho Dropdown)
  function buildScrollTags(){
    const box=$('#mta-sugs'); if(!box) return;
    box.innerHTML = `
      <button class="sug-tag" data-q="Xe số">🏍️ Xe số</button>
      <button class="sug-tag" data-q="Xe ga">🛵 Xe ga</button>
      <button class="sug-tag" data-q="Xe điện">⚡ Xe điện</button>
      <button class="sug-tag" data-q="50cc">🚲 Xe 50cc</button>
      <button class="sug-tag" data-q="Xe côn tay">🏍️ Xe côn tay</button>
      <button class="sug-tag" data-q="Thủ tục">📄 Thủ tục</button>
      <button class="sug-tag" data-q="Liên hệ">☎️ Liên hệ</button>
    `;
    box.querySelectorAll('.sug-tag').forEach(b=>{
  ...
  function updateCtxWithUser(_){} // rút gọn ngữ cảnh (giữ nguyên)
  ...
  ready(()=>{
    ...
    injectUI(); 
    buildScrollTags(); // Thay thế buildDropdown()
    ...
    // === Thêm: Auto-hide scrolltags khi focus (an toàn cho iOS) ===
    const sugsBox = $('#mta-sugs');
    const inBox = $('#mta-in');
    if(sugsBox && inBox){
      inBox.addEventListener('focus', () => {
        // Ẩn mượt
        sugsBox.style.maxHeight = '0';
        sugsBox.style.paddingTop = '0';
        sugsBox.style.paddingBottom = '0';
        sugsBox.style.opacity = '0';
      });
      inBox.addEventListener('blur', () => {
        // Hiện lại mượt
        sugsBox.style.maxHeight = '60px'; // Giá trị đủ lớn cho 1 hàng
        sugsBox.style.paddingTop = '8px';
        sugsBox.style.paddingBottom = '8px';
        sugsBox.style.opacity = '1';
      });
    }
    // === Hết phần auto-hide ===
  ...
  // ===== Expose (mini API)
  window.MotoAI_v22c_messenger_scrolltags = { // Đổi tên để tránh xung đột
