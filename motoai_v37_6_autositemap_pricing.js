/* motoai_v37_6_autositemap_pricing.js
   ...
   Brand: Nguyen Tu
   Note: BẢN GỘP — input nhỏ + vá path-aware + fix label CFG.brand
*/
(function(){
  ...
  /* ====== SESSION ====== */
  function getSess(){ const arr = safeJSON(localStorage.getItem(K.sess)) || []; return Array.isArray(arr) ? arr : []; }
  function saveSess(arr){ try{ localStorage.setItem(K.sess, JSON.stringify(arr.slice(-MAX_MSG))); }catch{} }
  ...
  function renderSess(){
    ...
    else addMsg("bot", naturalize(`Xin chào 👋, em là nhân viên hỗ trợ của ${CFG.brand}. Anh/chị cần thuê xe số, xe ga hay theo tháng ạ?`));
  }
  ...
  /* ====== AUTO-LEARN: ƯU TIÊN moto_sitemap.json (path-aware) ====== */
  async function learnFromSitemapOrSite(){
    ...
    // 1) thử moto_sitemap.json ở thư mục hiện tại, rồi fallback gốc origin
    const BASE = new URL('.', location.href).href.replace(/\/+$/,'/'); // current dir with trailing slash
    const sitemapPrimary  = new URL('moto_sitemap.json', BASE).href;
    const sitemapFallback = new URL('moto_sitemap.json', location.origin + '/').href;

    async function fetchJson(u){ try{ const r = await fetch(u); if(!r.ok) return null; return await r.json(); }catch{return null;} }

    let chosenUrl = null;
    let json = await fetchJson(sitemapPrimary);
    if(json){ chosenUrl = sitemapPrimary; } else {
      json = await fetchJson(sitemapFallback);
      if(json) chosenUrl = sitemapFallback;
    }
    ...
    if (json){
      try{
        ...
        if(grouped.length){
          cache["sitemap-json"] = {domain:chosenUrl, ts:nowSec(), pages: grouped};
          saveLearn(cache);
          localStorage.setItem(K.stamp, Date.now());
          return cache;
        }
      }catch(e){ console.warn("MotoAI: sitemap-json parse error", e); }
    }

    // 2) fallback: sitemap/crawl cũng path-aware
  ...
  async function deepAnswer(userText){
    ...
    if(intents.needDocs)    return polite(`thủ tục gọn: CCCD/hộ chiếu + cọc theo xe. Không để giấy tờ có thể thêm 500k thay giấy tờ.`);
    if(intents.needPolicy)  return polite(`đặt cọc tham khảo: xe số 2–3 triệu; xe ga 2–5 triệu; 50cc cọc 4 triệu. Liên hệ Zalo ${CFG.phone} để chốt theo mẫu xe.`);
    if(intents.needDelivery)return polite(`thuê 1–4 ngày vui lòng đến cửa hàng để chọn xe; thuê tuần/tháng em giao tận nơi. Phí nội thành 20–100k tuỳ quận. Nhắn Zalo ${CFG.phone} để em set lịch.`);
  ...
  function showTyping(){
    ...
    box.id = "mta-typing"; box.innerHTML = `<span>Đang nhập</span>`;
  ...
  async function sendUser(text){
    ...
    const ans = await deepAnswer(v); hideTyping(); addMsg("bot", ans);
  ...
  ready(async ()=>{
    ...
    if (!lastClean || (Date.now()-lastClean) > 7*24*3600*1000){
      ...
      localStorage.setItem(K.clean, Date.now()); console.log("MotoAI v37.6: cache cleaned");
    }
    ...
    if(CFG.autolearn){
      try{ await learnFromSitemapOrSite(); console.log("%cMotoAI v37.6 — learned from sitemap/site","color:"+CFG.themeColor+";font-weight:bold;"); }
