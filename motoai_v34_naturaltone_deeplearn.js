/* motoai_v34_naturaltone_deeplearn.js
   ...
   - NaturalTone: loại các hạt từ kiểu “ạ/nhé ạ/ạ nha/ạ nhé”, văn phong tự nhiên, lịch sự
   - Né quick-call/appbar/keyboard, không làm trắng menu, không phá layout
*/
(function(){
  if (window.MotoAI_v34_LOADED) return; window.MotoAI_v34_LOADED = true;
  ...
  const DEF = {
    ...
    autolearn: true,
    deepContext: true,
    maxContextTurns: 7,     // v34 tăng trí nhớ 7 lượt
    viOnly: true,
  ...
  // ===== NaturalTone: loại các hạt từ “ạ/nhé ạ/ạ nha/ạ nhé” & làm gọn câu
  function naturalTone(s){
    if(!s) return s;
    // bỏ biến thể có dấu/không dấu, có khoảng trắng
    s = s.replace(/\bnhé\s*ạ\b/gi, "nhé")
         .replace(/\bạ\s*nhé\b/gi, "nhé")
         .replace(/\bạ\s*nha\b/gi, "nha")
         .replace(/\bnha\s*ạ\b/gi, "nha")
         .replace(/\bạ\b/gi, ""); // bỏ đơn “ạ”
    // dọn khoảng trắng và dấu câu
    s = s.replace(/\s{2,}/g, " ").replace(/\s+([,.!?])/g, "$1").trim();
    // tránh kết thúc “nhé.” “nha.” lặp dấu
    s = s.replace(/(nhé|nha)[.?!]+$/i, (_,w)=> w + ".");
    return s;
  }

  // ===== Storage keys
  const K = { sess:'MotoAI_v34_session', ctx:'MotoAI_v34_ctx', learn:'MotoAI_v34_learn' };

  // ===== UI (Messenger giữ nguyên, có theme)
  const css = `
  ...
  function renderSess(){
    ...
    else addMsg('bot', naturalTone(`Xin chào 👋, em là nhân viên hỗ trợ của ${CFG.brand}. Anh/chị cần xem Xe số/ Xe ga/ Xe điện/ Thủ tục hay Bảng giá?`));
  }
  ...
  const formatRange = arr => !arr?.length ? null : (arr.length===1? nfVND(arr[0])+'đ' : nfVND(arr[0])+'–'+nfVND(arr[1])+'đ');
  ...
  function baseFor(type,unit){
    ...
    const arr=it[key]; return arr?arr[0]:null;
  }
  ...
  const SUFFIX = [
    ".", " nhé.", " nha.", ". Cảm ơn anh/chị."
  ];
  function polite(s){
    s=(s||"").trim();
    if(!s) s="em chưa nhận được câu hỏi, anh/chị thử nhập lại giúp em nhé.";
    return naturalTone(`${pick(PREFIX)} ${s}${pick(SUFFIX)}`);
  }
  ...
  function composePrice(type, qty){
    ...
    if(!base)  return polite(`Giá theo ${qty.unit} của ${type} hiện chưa có trong bảng. Anh/chị liên hệ Zalo ${CFG.phone} để báo giá chính xác nhé.`);
  ...
  function deepAnswer(userText){
    ...
    if(it.needDocs)    return polite(`thủ tục gọn: CCCD/hộ chiếu + cọc tuỳ xe. Có phương án giảm cọc khi đủ giấy tờ. Em có thể hướng dẫn nhận/trả xe nếu cần.`);
    if(it.needPrice)   return composePrice(type, qty);

    if(/(chào|xin chào|hello|hi|alo)\b/i.test(lower)){
  ...
  const parseXML = t=>{ try{ return (new DOMParser()).parseFromString(t,'text/xml'); }catch{ return null; } };
  const parseHTML= t=>{ try{ return (new DOMParser()).parseFromString(t,'text/html'); }catch{ return null; } };
  ...
  async function learnOneSite(origin){
    try{
      ...
      const uniq = Array.from(new Set(urls.map(u=>{ try{ return new URL(u).toString().split('#')[0]; }catch{ return null; } })
  ...
  function showTyping(){
    ...
    let i=0; typingBlinkTimer=setInterval(()=>{ dot.textContent='.'.repeat((i++%3)+1); }, 380); // v34 nhanh hơn chút
  }
  ...
  function openChat(){ if(isOpen) return; $('#mta-card').classList.add('open'); $('#mta-backdrop').classList.add('show'); $('#mta-bubble').style.display='none'; isOpen=true; renderSess(); setTimeout(()=>{ try{$('#mta-in').focus();}catch{} }, 120); }
  ...
  function clearChat(){
    ...
    $('#mta-body').innerHTML='';
    addMsg('bot', naturalTone('Đã xóa hội thoại, em sẵn sàng hỗ trợ lại.'));
  }
  ...
  async function sendUser(text){
    ...
    showTyping(); const typingDelay = 1800 + Math.random()*1400; await sleep(typingDelay);
    ...
    hideTyping(); addMsg('bot', naturalTone(ans));
    pushContext({from:'bot', raw:naturalTone(ans)});
    sending=false;
  }
  ...
  ready(async ()=>{
    ...
    console.log('%cMotoAI v34 NaturalTone-DeepLearn — Ready','color:'+CFG.themeColor+';font-weight:bold;');

    if(CFG.autolearn){
      const sites = Array.from(new Set([location.origin, ...(CFG.extraSites||[])]));
      try{ (async()=>{ await learnSites(sites, false); console.log('MotoAI v34 autolearn: done'); })(); }
      catch(e){ console.warn('MotoAI v34 autolearn err', e); }
    }
  });
