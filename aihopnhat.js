/* motoai_merged_v10_ui_v13_engine.js
   MOTOAI MERGED VERSION
   UI/CSS: v10 (Stable)
   Engine/Logic: v13 (Smart)
*/

/* ===========================
   PART 1/3 — UI (from v10) + CORE (from v13)
   =========================== */
(function(){
  ...
  console.log('✅ MotoAI Merged (v10-UI, v13-Engine) — initializing UI core...');
  ...
  /* --------- HTML inject (from v10 - Stable UI) ---------- */
  const html = `
  ...
  /* Chỗ này v10 có #motoai-typing min-width: 48px, nhưng v13 không có. Giữ theo v13 (không có) để khớp logic v13 */
  ...
  /* Nút clear của v10 bị đè, dùng v13 */
  ...
  /* Thêm style cho typing dots (từ v13) vì CSS v10 không có */
  ...
  /* Thêm style cho glow và shake (từ v13) */
  .m-msg.bot.glow{
...
/* ===========================
   PART 2/3 — Core logic, corpus, retrieval, session, init (from v13)
   =========================== */
  ...
  // Đây là bản gốc v13, nó sẽ được ghi đè bởi `integrateSmartEngine`
...
const rules = [
  ...
  { pattern: /^(cảm ơn|thanks|ok|oke|tuyệt vời|tốt quá|hay quá)$/i,
    answer: [
      "Không có gì ạ! Bạn cần hỗD'où ỗ trợ gì thêm cứ hỏi mình nhé. 😊",
...
(function(){
  const setTheme = ()=>{
    ...
    if(isDark){
      // Dùng var từ v10 CSS
      r.style.setProperty('--m10-card-bg', 'var(--m10-card-bg-dark)');
      document.body.dataset.theme='dark';
    }else{
      // Dùng var từ v10 CSS
      r.style.setProperty('--m10-card-bg', 'rgba(255,255,255,0.86)');
...
/* ---------- Global Light Mode Fix (from v10) ---------- */
(function(){
  const cssFix = `
...
function init(){
  ...
  console.log('%c⚙️ MotoAI v13 Combo (Merged) — UI & Core initialized', 'color:#0a84ff');
}
...
// final log
console.log('%c🚀 MotoAI Merged (v10-UI, v13-Engine) Standalone — Full Smart + Adaptive + UI', 'color:#0a84ff;font-weight:bold;');
