// MotoAI v9.7 Stable+ (Left Dock Edition) 😎
// Giao diện mượt, 4 tag, tối ưu Safari iOS, tránh QuickCall
window.addEventListener('DOMContentLoaded', () => {
  ...
  <div id="motoai-root">
    ...
    <div id="motoai-card" aria-hidden="true">
      ...
      <div id="motoai-body">
        <div class="m-msg bot">👋 Xin chào! Mình là MotoAI — hỏi thử “Xe ga” hay “Xe 50cc” nhé!</div>
      </div>
  ...
  function dummyRetrieve(q) {
    ...
    if (q.includes('50cc')) return '🚲 Xe 50cc là lựa chọn lý tưởng cho người chưa có bằng lái nhé.';
    if (q.includes('ga')) return '🛵 Xe ga chạy êm, phù hợp nội thành.';
    if (q.includes('số')) return '🏍 Xe số tiết kiệm, bền bỉ cho đi xa.';
    if (q.includes('thủ tục')) return '📄 Thủ tục thuê xe rất đơn giản: chỉ cần CCCD + cọc nhỏ.';
    return corpus.slice(0, 120) + '...';
  }
  ...
  function clearChat() {
    state.msgs = [{ role: 'bot', text: '🗑 Đã xóa hội thoại.' }];
    renderMsgs();
  }
  ...
  document.querySelectorAll('#motoai-suggestions button').forEach(b => {
    b.onclick = () => sendMessage(b.dataset.q);
  });

  console.log('✅ MotoAI v9.7 Stable+ loaded.');
});
