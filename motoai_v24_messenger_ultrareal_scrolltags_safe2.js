/* motoai_v24_messenger_ultrareal_scrolltags_safe2.js
   Messenger-style ~95% • Scrollable Tag Bar • SmartCalc • UltraSafe v2 (iOS/Safari/Light-mode)
   Brand: Motoopen | Phone/Zalo: 0857255868 | Map: https://maps.app.goo.gl/2icTBTxAToyvKTE78
*/
(function(){
  if(window.MotoAI_v24_MESSENGER_LOADED) return;
  window.MotoAI_v24_MESSENGER_LOADED = true;
  ...
  <div id="mta-root" aria-live="polite">
    ...
    <section id="mta-card" role="dialog" aria-label="Chat MotoAI" aria-hidden="true">
        ...
        <div class="brand">
          <div class="left">
            ...
            <div class="info">
              ...
              <div class="sub" id="mta-status">Đang hoạt động 🟢</div>
            </div>
          </div>
          <nav class="quick">
      ...
      <footer id="mta-input">
        ...
        <button id="mta-send" aria-label="Gửi" title="Gửi"><span class="plane">➤</span></button>
      </footer>
    </section>
  </div>`;
  ...
  #mta-body{flex:1;overflow:auto;padding:14px 12px;background:#E9EEF5;-webkit-overflow-scrolling:touch;overscroll-behavior:contain}
  ...
  // ==== Open/Close (an toàn iOS/Safari + debounce + fallback)
  function forceReflow(el){ try{ void el.offsetHeight; }catch(_){} }
  function openChat(){
    if(isOpen || bubbleLock) return;
    bubbleLock = true; setTimeout(()=>bubbleLock=false, 360);

    try{
      const card = $('#mta-card'), backdrop = $('#mta-backdrop'), bubble = $('#mta-bubble');
      // Safari glitch fix: reflow trước khi toggle class
      forceReflow(card);
      ...
      if(!IS_IOS) setTimeout(()=>{ try{$('#mta-in')?.focus();}catch(e){} }, 140);
      // IS_IOS: không auto-focus để tránh lock viewport

      document.body.style.overflow='hidden'; // chống scroll nền
      isOpen=true; renderSess();

      // Fallback khôi phục nếu vì lý do nào đó không mở được
      setTimeout(()=>{
  ...
  // ==== Status dot (xanh giờ hành chính, xám ngoài giờ)
  function updateStatus(){
    const el = $('#mta-status'); if(!el) return;
    const h = new Date().getHours();
    const online = (h >= 7 && h <= 22);
    el.textContent = online ? 'Đang hoạt động 🟢' : 'Ngoài giờ 🔘';
  }
  ...
  ready(()=>{
    ...
    $('#mta-send').addEventListener('click', ()=>{ const v=($('#mta-in').value||'').trim(); if(!v) return; $('#mta-in').value=''; sendUser(v); }, {passive:true});
    ...
    document.addEventListener('keydown', (e)=>{ if(e.key==='Escape') closeChat(); }, {passive:true});
    ...
    // Watchdog
    setTimeout(()=>{ if(!$('#mta-bubble')) injectUI(); }, 2200);

    console.log('%cMotoAI v24 Messenger UltraReal Safe2 — Active','color:#0084FF;font-weight:bold;');
  });

  // ==== Expose
  window.MotoAI_v24_messenger = {
