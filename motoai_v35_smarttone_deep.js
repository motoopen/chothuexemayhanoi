/* motoai_v35_smarttone_deep.js
   UI Messenger (giữ ổn định) + SmartContext-Deep + VN-only + AutoLearn (sitemap + RSS)
   - Loại bỏ hẳn "ạ / nhé / nha" bằng bộ lọc SmartTone (stripPoliteParticles)
   - Học 5 nguồn (site) + RSS fanpage, làm mới 12h, an toàn trình duyệt
   - Né quick-call / appbar / keyboard; không auto-focus input (iOS friendly)
*/
(function(){
  if (window.MotoAI_v35_LOADED) return; window.MotoAI_v35_LOADED = true;

  // ========= Config (mặc định, có thể override bằng window.MotoAI_CONFIG) =========
  const DEF = {
    brand: "Motoopen",
    phone: "0857255868",
    zalo:  "",
    ...
    extraSites: [
      location.origin
      // gợi ý: có thể thêm site và RSS feed ở window.MotoAI_CONFIG.extraSites
    ],
    crawlDepth: 1,
    refreshHours: 12,
  ...
  // SmartTone: loại bỏ hạt từ lịch sự rườm rà
  function stripPoliteParticles(text){
    if(!text) return text;
    // xóa các biến thể đứng độc lập, trước khi chấm/cuối câu
    return text
      .replace(/\s+(ạ|nhé|nha)(?=[\s\.\,\!\?\)]|$)/gi, '')
      .replace(/\s{2,}/g,' ')
      .replace(/\s+\./g,'.')
      .trim();
  }

  // VN-only: rule nhẹ (không can thiệp câu trả lời, chỉ đảm bảo thuần Việt khi có thể)
  function enforceVietnamese(text){ return CFG.viOnly ? text : text; }

  // ========= Storage Keys =========
  const K = {
    sess: 'MotoAI_v35_session',
    ctx:  'MotoAI_v35_ctx',
    learn:'MotoAI_v35_learn'
  };
  ...
  function renderSess(){
    ...
    else addMsg('bot', stripPoliteParticles(enforceVietnamese(
      `Xin chào 👋, em là nhân viên hỗ trợ của ${CFG.brand}. Anh/chị cần xem Xe số, Xe ga, Xe điện hay Thủ tục/Bảng giá?`
    )));
  }
  ...
  function readContext(){ return safe(localStorage.getItem(K.ctx)) || { turns:[] }; }
  ...
  // ========= Compose (giọng tự nhiên) =========
  const PREFIX = ["Chào anh/chị,", "Xin chào 👋,", "Em chào anh/chị,", "Rất vui được hỗ trợ anh/chị,"];
  const SUFFIX = [".", ". Cảm ơn anh/chị."]; // không có "ạ/nhé/nha"
  function polite(s){ s=(s||"").trim(); if(!s) s="em chưa nhận được câu hỏi, anh/chị thử nhập lại giúp em."; return `${pick(PREFIX)} ${s}${pick(SUFFIX)}`; }
  ...
  function deepAnswer(userText){
    ...
    if(it.needContact) return polite(`anh/chị có thể nhắn Zalo ${CFG.phone} hoặc gọi ${CFG.phone} để được tư vấn nhanh.`);
  ...
  function composePrice(type, qty){
    if(!type) type = 'xe số';
    if(!qty)  return polite(`Giá ${type} khoảng ${summariseType(type)}. Anh/chị thuê mấy ngày để em ước tính tổng?`);
    const base = baseFor(type, qty.unit);
    if(!base)  return polite(`Giá theo ${qty.unit} của ${type} chưa có sẵn. Anh/chị liên hệ Zalo ${CFG.phone} để lấy báo giá chính xác.`);
    const total = base * qty.n;
    const label = qty.unit==='ngày'?`${qty.n} ngày`:qty.unit==='tuần'?`${qty.n} tuần`:`${qty.n} tháng`;
    return polite(`Giá dự kiến thuê ${type} ${label} khoảng ${nfVND(total)}đ (ước tính). Anh/chị có thể liên hệ Zalo ${CFG.phone} để chốt xe.`);
  }

  // ========= AutoLearn (sitemap + RSS) =========
  ...
  // 🆕 Đọc RSS feed (title/link/description)
  async function readRSS(url){
    const xmlTxt = await fetchText(url); if(!xmlTxt) return [];
    const doc = parseXML(xmlTxt); if(!doc) return [];
    // Nếu không có <item> thì không phải RSS feed hợp lệ
    ...
    return items.map(it=>{
      ...
      const txt   = (title+' '+desc).replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim();
      return {url: link||url, title, text: txt};
  ...
  async function learnOneSite(origin){
    try{
      // Nếu là file XML → thử RSS trước, nếu không phải RSS thì đọc như sitemap
      if(/\.xml(\?|$)/i.test(origin)){
        const rss = await readRSS(origin);
        if(rss && rss.length){ return {domain: origin, ts: nowSec(), pages: rss}; }
        const urlsFromSitemap = await readSitemap(origin);
        const uniq = Array.from(new Set(urlsFromSitemap));
  ...
  async function learnSites(listOrigins, force=false){
    ...
    for(const origin of origins){
      try{
        const key = /\.xml(\?|$)/i.test(origin) ? origin : new URL(origin).origin;
        ...
        const data = await learnOneSite(origin);
  ...
  function suggestFromIndex(q){
    try{
      ...
      const score = it=>{
        ...
        let s = 0; if(text.includes('giá')) s++; if(text.includes('thuê')) s++; if(text.includes('xe')) s++; if(text.includes('hà nội')) s++;
      ...
      return `${snippet} ... Xem thêm: ${cand.url}`;
    }catch{ return null; }
  }
  ...
  function openChat(){ if(isOpen) return; $('#mta-card').classList.add('open'); $('#mta-backdrop').classList.add('show'); $('#mta-bubble').style.display='none'; isOpen=true; renderSess(); /* không focus input để tránh keyboard iOS */ }
  ...
  function clearChat(){
    ...
    $('#mta-body').innerHTML='';
    addMsg('bot', 'Đã xóa hội thoại, em sẵn sàng hỗ trợ lại.');
  }
  ...
  async function sendUser(text){
    ...
    showTyping(); const typingDelay = 1200 + Math.random()*900; await sleep(typingDelay);
    let ans = deepAnswer(userText);
    if(/chưa nhận được câu hỏi|quan tâm loại xe nào/i.test(ans||'')){
      const sug = suggestFromIndex(userText);
      if(sug) ans = polite(sug);
    }
    hideTyping();
    ans = stripPoliteParticles(enforceVietnamese(ans));
    addMsg('bot', ans);
    pushContext({from:'bot', raw:ans});
    sending=false;
  }

  // ========= Né quick-call / keyboard =========
  ...
  ready(async ()=>{
    ...
    // re-disable map if DOM thay đổi
    const mo = new MutationObserver(()=>{ disableMapIfNeeded(); });
    mo.observe(document.body, {childList:true, subtree:true});
    setTimeout(disableMapIfNeeded, 400);

    console.log('%cMotoAI v35 SmartTone-Deep — Ready','color:'+CFG.themeColor+';font-weight:bold;');

    if(CFG.autolearn){
      const sites = Array.from(new Set([...(CFG.extraSites||[])]));
      try{ (async()=>{ await learnSites(sites, false); console.log('MotoAI v35 autolearn: done'); })(); }
      catch(e){ console.warn('MotoAI v35 autolearn err', e); }
    }
  });
  ...
  window.MotoAI_v35 = {
    ...
    learnNow: async (sites, force)=>{ try{ const list = Array.isArray(sites)&&sites.length?sites:(CFG.extraSites||[]); return await learnSites(list, !!force); }catch(e){ return null; } },
    getIndex: ()=> getIndex(),
