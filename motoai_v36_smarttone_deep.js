/* motoai_v36_smarttone_deep.js
   Messenger UI + SmartContext-Deep + VN-only + Auto-avoid
   - BỎ HOÀN TOÀN gợi ý link trong chat
   - Độ trễ "đang nhập" 3–6s
   - Giọng tự nhiên: loại "ạ/nhé/nha" và các hạt tương tự
*/
(function(){
  if (window.MotoAI_v36_LOADED) return; window.MotoAI_v36_LOADED = true;
  ...
  // ===== Storage keys
  const K = {
    sess: 'MotoAI_v36_session',
    ctx:  'MotoAI_v36_ctx',
    learn:'MotoAI_v36_learn'
  };
  ...
  ready(async ()=>{
    ...
    console.log('%cMotoAI v36 SmartTone-Deep — Ready','color:'+CFG.themeColor+';font-weight:bold;');

    if(CFG.autolearn){
      const inputs = Array.from(new Set([...(CFG.extraSites||[])]));
      try{
        (async()=>{ await learnSites(inputs, false); console.log('MotoAI v36 autolearn: done'); })();
      }catch(e){ console.warn('MotoAI v36 autolearn err', e); }
    }
  });
