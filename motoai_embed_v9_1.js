// MotoAI Embed v9.1 — Fixed iOS Auto-Close + Smooth Chat UX
(function(){
  if(window.MotoAI_EMBED_V9_1) return;
  window.MotoAI_EMBED_V9_1 = true;
  console.log("✅ MotoAI Embed v9.1 loaded (Fixed auto-close)");
  ...
  function close(force=false){
    if(!force && chatOpen) return; // không tự đóng khi đang chat
    chatOpen = false;
    ...
    const tryAsk = ()=>{
      if(window.MotoAI && typeof window.MotoAI.ask==="function"){
        window.MotoAI.ask(text, answer=>{
          tnode.remove();
          addMsg("bot", answer);
          setTimeout(()=>input.focus(), 300); // giữ khung chat luôn mở
        });
      } else {
        setTimeout(tryAsk, 300);
      }
    };
    tryAsk();
  }
  ...
  coreScript.onload = ()=>{
    setTimeout(()=>{
      if(window.MotoAI && MotoAI.getUserName){
