/*
 * 🤖 MotoAI v18p — UI 9.8 PoliteClean + AutoLearn
 * - Không dùng “ạ / dạ / vâng”, chỉ “bạn / mình” lịch sự, tự nhiên
 * - UI98 bong bóng góc trái, auto dark/light
 * - Header: "@ AI Assistant — Tel: 0857 255 868"
 * - Gợi ý: 💰 Bảng giá | ⚙️ Dịch vụ | 🏍️ Sản phẩm | ☎️ Liên hệ
 * - AutoLearn: DOM hiện tại + *_sitemap.json + link nội bộ (giới hạn)
 * - Lưu corpus theo domain (localStorage), tự làm mới mỗi 24h
 */
(function(){
  if (window.MotoAI_v18p_LOADED) return;
  window.MotoAI_v18p_LOADED = true;
  console.log('%cMotoAI v18p UI98 PoliteClean loading…','color:#0a84ff;font-weight:700');
  ...
  const CFG = {
    ...
    maxInternalPages: 20,
    refreshHours: 24,
    corpusKey: `MotoAI_v18p_${HOSTKEY}_corpus`,
    extCorpusKey: `MotoAI_v18p_${HOSTKEY}_corpus_ext`,
    lastLearnKey: `MotoAI_v18p_${HOSTKEY}_lastLearn`,
    lastSitemapHashKey: `MotoAI_v18p_${HOSTKEY}_lastSitemapHash`,
    sessionKey: `MotoAI_v18p_${HOSTKEY}_session`
  };
  ...
  function buildCorpusFromDOM(){
    try{
      ...
      saveCorpus();
      console.log('📚 v18p: Built DOM corpus:', corpus.length);
  ...
  window.MotoAI_v18p = {
    ...
    version: 'v18p-ui98-politeclean'
  };

  console.log('%c✅ MotoAI v18p ready — window.MotoAI_v18p','color:#0a84ff;font-weight:700');
})();
