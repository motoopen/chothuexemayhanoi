// MotoAI v4.1 Stable by Motoopen 😎
// Fix: màu chữ / nền sáng-tối, iPhone focus, 3 nút gợi ý
window.addEventListener("DOMContentLoaded", () => {
  ...
  const state = { msgs: [{ role:'bot', text:'Chào bạn 👋! Mình là MotoAI v4.1 — hỏi thử điều gì đó nhé.' }] };
  ...
  function findAnswer(q) {
    ...
    if (q.includes('xe ga')) return 'Xe ga phù hợp di chuyển trong phố 🛵, nhẹ nhàng và tiện lợi.';
    if (q.includes('xe số')) return 'Xe số bền, tiết kiệm xăng 🏍 — rất hợp đi xa.';
    if (q.includes('thủ tục')) return 'Thủ tục thuê xe đơn giản 📄, chỉ cần CCCD và cọc nhẹ.';
  ...
  render();
  console.log("✅ MotoAI v4.1 Loaded & fixed colors!");
});
