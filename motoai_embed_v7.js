// MotoAI v7.0 — Local Learning Chat + Nút đóng + Gợi ý thông minh 😎
window.addEventListener('DOMContentLoaded', function() {
  if (window.MotoAI_v7_LOADED) return;
  window.MotoAI_v7_LOADED = true;
  ...
  #motoai-root{position:fixed;left:16px;bottom:80px;z-index:900}
  ...
  #motoai-backdrop{position:fixed;inset:0;background:rgba(0,0,0,0.25);opacity:0;pointer-events:none;transition:opacity .3s;z-index:899}
  #motoai-backdrop.show{opacity:1;pointer-events:auto}
  #motoai-card{position:fixed;left:0;right:0;bottom:0;width:min(900px,calc(100% - 30px));margin:auto;height:70vh;max-height:720px;border-radius:18px 18px 0 0;background:var(--bg-light);box-shadow:0 -12px 40px rgba(0,0,0,.18);transform:translateY(110%);opacity:0;display:flex;flex-direction:column;overflow:hidden;z-index:901;transition:transform .4s cubic-bezier(.2,.9,.2,1),opacity .3s}
  ...
  console.log('✅ MotoAI v7 loaded (UI + Local Learning).');
});
