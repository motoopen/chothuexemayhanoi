/* motoai_v23c_messenger_ultrareal_scrolltags.js
   Messenger-style ~95% • Scrollable Tag Bar • SmartCalc • UltraSafe • LEFT quick-call avoid
   Brand: Motoopen | Zalo/Phone: 0857255868 | Map: https://maps.app.goo.gl/2icTBTxAToyvKTE78
   Patch: move to LEFT, safer open/close, improved auto-avoid & iOS keyboard handling
*/
(function(){
  ...
  // ===== UI — Messenger 95% + Tag bar kéo ngang (LEFT position)
  const ui = `
  ...
  #mta-root{
    ...
    transition:bottom .25s ease,left .25s ease;
  }
  ...
  #mta-card{position:fixed;left:16px;right:auto;bottom:16px;width:min(420px,calc(100% - 24px));height:70vh;max-height:740px;background:var(--mta-bg);color:var(--mta-text);border-radius:18px;box-shadow:0 14px 40px rgba(0,0,0,.25);transform:translateY(110%);opacity:.99;display:flex;flex-direction:column;overflow:hidden;transition:transform .20s cubic-bezier(.22,1,.36,1)}
  ...
  @media(max-width:520px){
    #mta-card{width:calc(100% - 16px);left:8px;right:8px;height:72vh}
  ...
  // ===== SAFER openChat / closeChat (LEFT-friendly, iOS-safe) -----
  function openChat(){
    try{ injectUI(); }catch(e){ console.warn('injectUI fail', e); }
    const card = $('#mta-card'), backdrop = $('#mta-backdrop'), bubble = $('#mta-bubble');
    if(!card || !bubble){
      console.warn('MotoAI: missing DOM nodes', !!card, !!bubble);
      return;
    }

    // mở card trước — chỉ ẩn bubble khi card thực sự mở
    ...
    // chờ animation frame rồi ẩn bubble nếu card đã mở
    requestAnimationFrame(()=>{
      setTimeout(()=>{
        ...
        try{ $('#mta-in') && $('#mta-in').focus(); }catch(e){ /* no-op */ }
      }, 80);
    });
  }
  function closeChat(){
    const card = $('#mta-card'), backdrop = $('#mta-backdrop'), bubble = $('#mta-bubble');
    try{ $('#mta-in') && $('#mta-in').blur(); }catch(e){}
  ...
  // ===== Tag bar events (safer) -----
  function bindTags(){
    const track = $('#tagTrack'); if(!track) return;
    track.querySelectorAll('button').forEach(b=>{
      b.addEventListener('click', ()=> {
        try{ sendUser(b.dataset.q); }catch(e){ console.warn('sendUser error', e); }
      });
    });
    const updateFade=()=>{
  ...
  // ===== Auto-avoid obstacles (left-aware) -----
  function checkObstacles(){
    const root=$('#mta-root'); if(!root) return;
    // try find blockers on left or bottom/right
    const leftBlock = document.querySelector('.quick-call.left, .mini-quick-left, .quick-call');
    const rightBlock = document.querySelector('.bottom-appbar, #quick-call, .quick-call.right');
    let bottom='calc(18px + env(safe-area-inset-bottom, 0))';
    let left = '16px';
    let right = 'auto';
    try{
      if(leftBlock){
        const r = leftBlock.getBoundingClientRect();
        if(r && !isNaN(r.top)){
          const space = Math.max(0, window.innerHeight - r.top);
          if(space < 120) bottom = (space + 70) + 'px';
        }
        try{
          // if leftBlock is near left edge, shift chat to its right
          const lbRight = leftBlock.getBoundingClientRect().right || 0;
          if(lbRight < 120){
            left = (Math.max(16, Math.round(lbRight + 12))) + 'px';
          }
        }catch(e){}
      }
      if(rightBlock){
        const r2 = rightBlock.getBoundingClientRect();
        if(r2 && !isNaN(r2.top)){
          const space2 = Math.max(0, window.innerHeight - r2.top);
          if(space2 < 120){
            bottom = (space2 + 70) + 'px';
          }
        }
      ...
      if(window.visualViewport){
        ...
        if(vv.height < window.innerHeight - 120){
          bottom = (window.innerHeight - vv.height + 20) + 'px';
        }
      }
    }catch(e){ console.warn('checkObstacles error', e); }
  ...
  // ===== fix Safari/iOS keyboard behavior (safer) -----
  function fixSafariKeyboard(){
    const card = $('#mta-card');
    if(!card || !window.visualViewport) return;
    let lastHv = window.visualViewport.height;
    window.visualViewport.addEventListener('resize', ()=>{
      try{
        const vv = window.visualViewport;
        // if viewport height decreased by >80px => keyboard probably opened
        if(vv.height < lastHv - 80){
          card.style.transform = 'translateY(0)';
          if(card.classList.contains('open')) $('#mta-bubble').style.display = 'none';
        } else {
          card.style.transform = card.classList.contains('open') ? 'translateY(0)' : 'translateY(110%)';
        }
        lastHv = vv.height;
      }catch(e){ /* swallow */ }
    }, {passive:true});
  ...
  ready(()=>{
    ...
    if(window.visualViewport) window.visualViewport.addEventListener('resize', ()=>{
      checkObstacles(); // keep it in sync with keyboard changes
    }, {passive:true});
    fixSafariKeyboard();

    // Watchdog: re-inject if removed
    setTimeout(()=>{ if(!$('#mta-bubble')) injectUI(); }, 2500);

    console.log('%cMotoAI v23c Messenger UltraReal (LEFT patch) — Active','color:#0084FF;font-weight:bold;');
  });
