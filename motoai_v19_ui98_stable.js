(function(){
  if (window.MotoAI_v19_LOADED) return; window.MotoAI_v19_LOADED = true;
  ...
  // ======= UI (không blur/gradient, hiệu ứng nhẹ; icon Messenger) =======
  const ui = `
    ...
    <section id="mta-card" role="dialog" aria-label="AI chat" aria-hidden="true">
      <header id="mta-header">
        <div class="brand">
          ...
          <nav class="quick">
            <a class="q q-phone"   href="tel:${CFG.phone}" title="Gọi"><span>📞</span></a>
  ...
  function showTyping(){ const d=document.createElement('div'); d.id='mta-typing'; d.className='m-msg bot'; d.textContent='...'; $('#mta-body').appendChild(d); $('#mta-body').scrollTop=$('#mta-body').scrollHeight; }
  ...
  function buildFromDOM(){
    try{
      ...
      save();
      console.log('📚 v19: DOM corpus =', corpus.length);
  ...
  function polite(t){ t=(t||"").trim(); if(!t) return "Mình chưa nhận được câu hỏi, bạn thử nhập lại nhé."; return /[.!?…]$/.test(t)? `${pick(PREFIX)} ${t} ${pick(SUFFIX)}` : `${pick(PREFIX)} ${t}${pick(SUFFIX)}`; }
  ...
  function retrieve(q){
    ...
    let best={s:0,t:null}; const pool=(corpus||[]).concat(ext||[]);
    for(const it of pool){
      const line = typeof it==='string'? it : it.text; const low=(line||'').toLowerCase();
      let s=0; for(const w of qt){ if(low.includes(w)) s++; }
      if(s>best.s) best={s,text:line};
    }
  ...
  // ======= Open/Close =======
  function openChat(){
    if(isOpen) return; $('#mta-card').classList.add('open'); $('#mta-backdrop').classList.add('show'); $('#mta-bubble').style.display='none'; isOpen=true; renderSess(); setTimeout(()=>{ try{$('#mta-in').focus()}catch(e){} },120);
  }
  function closeChat(){
  ...
  function buildSugs(){
    const box=$('#mta-sugs'); box.innerHTML=''; SUGS.forEach(s=>{ const b=document.createElement('button'); b.type='button'; b.textContent=s.label; b.addEventListener('click',()=>{ if(!isOpen) openChat(); setTimeout(()=> sendUser(s.q),80); }); box.appendChild(b); });
  }

  async function sendUser(text){
    if(sending) return; sending=true;
    addMsg('user', text); showTyping();
    await sleep(180 + Math.min(520, text.length*6));
    let ans; try{ ans = compose(text); }catch(e){ ans = null; }
    hideTyping(); addMsg('bot', ans || polite('xin lỗi, có lỗi khi trả lời. Bạn thử lại giúp mình')); sending=false;
  }
