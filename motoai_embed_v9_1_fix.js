// MotoAI Embed v9.1 FIX — tránh gửi nhiều lần, queue wait-for-core, suggestions trực tiếp
(function(){
  if(window.MotoAI_EMBED_V9_1_FIX) return;
  window.MotoAI_EMBED_V9_1_FIX = true;
  console.log("✅ MotoAI Embed v9.1 FIX loaded");

  const CORE_URL = "https://cdn.jsdelivr.net/gh/motoopen/chothuexemayhanoi@main/motoai_core_v9.js";

  // --- HTML (giữ nguyên cấu trúc cũ) ---
  const html = `
    ...
    <div id="motoai-v9-overlay" class="hidden" aria-hidden="true">
      <div id="motoai-v9-card" role="dialog" aria-modal="true">
        ...
        <main id="motoai-v9-body">
          <div class="m-msg bot">👋 Xin chào! Mình là MotoAI — hỏi thử "Xe ga", "Xe số", hoặc "Thủ tục" nhé.</div>
        </main>
  ...
  // --- CSS (giữ giống file trước để tránh style break) ---
  ...
  let chatOpen = false;
  let sendLock = false; // prevent duplicate sends
  ...
  // wait for core once (promise)
  let coreReadyResolve;
  const coreReady = new Promise(resolve => { coreReadyResolve = resolve; });
  coreScript.addEventListener('load', () => {
    coreReadyResolve(true);
  });
  // safety: also resolve after 2.5s even if core not present (fallback)
  setTimeout(()=> coreReadyResolve(true), 2500);
  ...
  bubble.addEventListener("click", ()=>{
    if(overlay.classList.contains("visible")) close(true); else open();
  });
  ...
  // Suggestions: call direct send to avoid double click issues
  ...
  // central send routine (for input field)
  ...
  async function sendQueryDirect(text){
    ...
    if(!text) return;
    if(sendLock) return; // drop duplicates
    sendLock = true;
    ...
    // wait for core to be ready (but not forever)
    await coreReady;

    // now call core.ask safely and once
    try{
      if(window.MotoAI && typeof window.MotoAI.ask === "function"){
        // protect core.ask from throwing
        window.MotoAI.ask(text, (answer)=>{
          try{
            if(typingNode && typingNode.remove) typingNode.remove();
            // guard against empty answer
            answer = (answer===null||answer===undefined) ? "Xin lỗi, mình chưa có câu trả lời." : String(answer);
            addMsg("bot", answer);
          }catch(e){
            console.warn("MotoAI display error", e);
            if(typingNode && typingNode.remove) typingNode.remove();
            addMsg("bot", "Lỗi hiển thị câu trả lời.");
          } finally {
            sendLock = false;
            send.disabled = false;
  ...
  // small iOS keyboard fix: adjust card transform by visualViewport offset
  (function fixIOSKeyboard(){
    ...
    visualViewport.addEventListener('resize', ()=>{
      try{
        ...
        // threshold to avoid tiny shifts
        if(Math.abs(offset - lastOffset) < 10) return;
        lastOffset = offset;
        if(offset > 120){
          // keyboard open -> lift card
  ...
  // try greet with stored name after core loaded
  coreScript.onload = ()=>{
    setTimeout(()=>{
      try{
