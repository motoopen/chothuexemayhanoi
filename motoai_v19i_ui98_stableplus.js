(function(){
  if (window.MotoAI_v19i_STABLEPLUS_LOADED) return; window.MotoAI_v19i_STABLEPLUS_LOADED = true;
  ...
  const K = {
    ...
    ctx:    `MotoAI_v19_${HOSTKEY}_ctxV2`
  };
  ...
  function buildFromDOM(){
    try{
      ...
      save();
      console.log('📚 stableplus: DOM corpus =', corpus.length);
  ...
  // Bắt topic từ câu gần nhất, ưu tiên thuê xe, loại xe, thủ tục, giá.
  const TOPIC_LEX = [
    ...
    {key:'gia',      kws:['giá','bảng giá','bao nhiêu','cost','price','thuê ngày','thuê tuần','thuê tháng']},
  ...
  function retrieve(q){
    ...
    const ctx = getCtx();           // topics: ['xe_so','gia',...]
    const ctxTokens = [];
    // ánh xạ topic -> từ khoá trọng số để bias
    const biasMap = {
    ...
    for(const it of pool){
      ...
      let s=0;
      // điểm theo truy vấn hiện tại
      for(const w of qt){ if(low.includes(w)) s+=1; }
      // điểm bias theo ngữ cảnh
  ...
  // ======= Suggestions (NEW set – tự wrap 2 hàng) =======
  const SUGS = [
  ...
  // ======= Send with typing delay 2–5s + cập nhật ngữ cảnh =======
  async function sendUser(text){
    if(sending) return; sending=true;
    addMsg('user', text);
    // cập nhật context theo user input
    try{ updateCtxWithUser(text); }catch(e){}
    showTyping();
    const typingDelay = 2000 + Math.random()*3000;  // 2–5s
  ...
  window.MotoAI_v19i_stableplus = {
    ...
    version: 'v19i-ui98-stableplus-sc2'
  };
})();
