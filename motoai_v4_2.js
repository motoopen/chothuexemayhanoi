// MotoAI v4.2 Smart Position 😎
// Tự động tránh nút nổi (Zalo, gọi, back-top), hỗ trợ iPhone + Dark Mode
window.addEventListener("DOMContentLoaded", () => {
  ...
  <div id="motoai-root">
    ...
    <div id="motoai-card" role="dialog" aria-modal="true">
      ...
      <div id="motoai-suggest">
        <button class="sug">🏍 Xe số</button>
        <button class="sug">🛵 Xe ga</button>
        <button class="sug">📄 Thủ tục</button>
      </div>
      <div id="motoai-input">
  ...
  #motoai-root {
    position:fixed;
    left:16px; bottom:120px; /* tạm mặc định */
    z-index:2147483647;
    pointer-events:none;
  ...
  const state = { msgs: [{ role:'bot', text:'Xin chào 👋! Mình là MotoAI v4.2 — hỏi thử điều gì đó nhé.' }] };
  ...
  function findAnswer(q) {
    ...
    if (w.includes('xe số')) return 'Xe số bền, tiết kiệm xăng 🏍 — phù hợp di chuyển xa.';
    if (w.includes('xe ga')) return 'Xe ga nhẹ, sang 🛵 — hợp phố xá và nữ giới.';
    if (w.includes('thủ tục')) return 'Thủ tục đơn giản 📄, chỉ cần CCCD và cọc nhẹ.';
    const seg = t.split(/[.?!]/).find(s => s.includes(w.split(' ')[0]));
    return seg ? seg.trim() : 'Mình chưa rõ lắm 🤔, bạn hỏi lại nhé.';
  }
  ...
  document.querySelectorAll('#motoai-suggest .sug').forEach(btn =>
  ...
  function adjustPosition() {
    ...
    const quick = document.querySelector('.quick-call-game, .quick-call');
    const back = document.querySelector('.back-top');
    if (quick) bottomOffset = 160;
    if (back) bottomOffset = 200;
    root.style.bottom = bottomOffset + 'px';
  }

  adjustPosition();
  window.addEventListener('resize', adjustPosition);
  console.log("✅ MotoAI v4.2 Smart Position loaded.");
});
