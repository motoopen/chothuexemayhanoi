/* motoai_v38_merged_semantic_pricing.js
   ✅ GỘP TÍNH NĂNG: v27 (Semantic BM25 + Extractive QA + Auto-Price Learn + Multi-site)
   ...
   - Semantic search (BM25 mini) + Extractive QA (chích câu “đinh”)
   - Tự học giá từ trang (Auto-Price Learn) + nhập về PRICE_TABLE (percentile)
   - Giọng tự nhiên (lọc “ạ/nhé/nha”), VN-only filter, DeepContext, UI nhỏ gọn
...
(function(){
  ...
  /* ====== PRICE TABLE + Auto-Price Learn (từ v27) ====== */
  const PRICE_TABLE = {
  ...
  // Trích giá từ HTML (đơn giản, giống v27)
  ...
  async function learnOneOrigin(origin){
    try{
      ...
      // 1) thử moto_sitemap.json ở gốc origin
      const candidatesJSON = [
        key + "/moto_sitemap.json",
        // path-aware: nếu trang nằm trong subpath, thử luôn thư mục hiện tại
          ...
          if(r && r.ok){
            ...
            for(const u of ds){
              ...
              if(/\.txt($|\?)/i.test(u)){
                ...
                // Auto-Price learn cũng áp dụng ở đây
                if(CFG.smart.autoPriceLearn){
                  const autos = extractPricesFromText(txt);
                  if(autos && autos.length){
                    const stash = safe(localStorage.getItem(K.autoprices))||[];
      ...
      // 2) sitemap.xml / index / RSS; 3) fallback crawl
      let urls=[];
  ...
  async function deepAnswer(userText){
    ...
    // Deep context: backfill type/qty từ lượt trước
    if(CFG.deepContext){
      const ctx = getCtx();
  ...
  ready(async ()=>{
    ...
    if(!lastClean || (Date.now()-lastClean) > 7*24*3600*1000){
      ...
      console.log("MotoAI v38: maintenance OK");
    }
    ...
    if(CFG.autolearn){
      try{
        ...
        if(!last || (Date.now()-last) >= CFG.refreshHours*3600*1000){
          ...
          console.log("%cMotoAI v38 — learned from multiple sites","color:"+CFG.themeColor+";font-weight:bold;");
        }else{
          console.log("MotoAI v38 — skip learn (fresh cache)");
        }
      }catch(e){ console.warn("MotoAI v38 autolearn error", e); }
    }
  });
