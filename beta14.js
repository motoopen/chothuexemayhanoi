...
(function(){
  // FIX: Kiểm tra window.MotoAI_v10.sendQuery có tồn tại không trước khi vá
  const checkAndPatch = () => {
    if (window.MotoAI_v10 && typeof window.MotoAI_v10.sendQuery === 'function') {
      ...
      window.MotoAI_v10.sendQuery = function(text){
        ...
        // Gọi hàm gốc với văn bản đã sửa
        origSend.call(this, fixed); 
      };
      console.log('%cMotoAI SpellFix enabled ✅', 'color:#0a84ff;font-weight:bold;');
    } else {
      // Nếu v10.sendQuery chưa sẵn sàng, thử lại sau
...
// 🌗 [ĐÃ XÓA] Khối đồng bộ Dark/Light mode (setTheme) đã bị xóa
// Lý do: Gây trùng lặp với hàm setTheme_Adaptive ở cuối file.
// Logic đã được hợp nhất vào hàm setTheme_Adaptive duy nhất.
...
window.addEventListener('load', ()=>{
  try{
    if(window.MotoAI_v10 && typeof window.MotoAI_v10.open === 'function'){
      ...
      // FIX: Đảm bảo 'init' tồn tại và là hàm trước khi gọi
      if(typeof init === 'function') {
        init();
      } else {
        console.error('💥 Không tìm thấy hàm init() để ép khởi động!');
      }
    }
  }catch(e){
...
// (function(){ // <-- ĐÃ GỠ BỎ IIFE LỒNG NHAU NÀY
  ...
  /* ---------- CSS (Đã áp dụng Dark Mode Apple Style VÀ FIX LỖI) ---------- */
  const css = `
    :root{
      /* --- BIẾN MÀU ĐƯỢC QUẢN LÝ BỞI JAVASCRIPT --- */
      /* (Các giá trị này là fallback cho Light Mode) */
      --m10-accent: #0a84ff; /* Xanh lam Apple */
      --m10-blur: blur(10px) saturate(130%);
      --m10-radius: 18px;
      
      --m10-card-bg: #ffffff;
      --bg: #ffffff; /* Màu nền input */
      --text: #000000; /* Màu chữ chính */
      --muted: #6e6e73; /* Màu chữ mờ, icon tools */
      
      --glass-border: rgba(0,0,0,0.08); /* Viền input, viền header/footer */
      --footer-bg: rgba(255,255,255,0.85); /* Nền footer */
      
      --m10-bot-bg: #f2f4f8; /* Nền tin nhắn bot */
      --m10-bot-text: #000000; /* Chữ tin nhắn bot */
      
      --m10-suggestion-bg: rgba(255,255,255,0.75); /* Nền thanh gợi ý */
      --m10-suggestion-button-bg: rgba(0,122,255,0.1); /* Nền nút gợi ý */
      --m10-suggestion-button-text: var(--m10-accent); /* Chữ nút gợi ý */
    }
    ...
    #motoai-root{
      ...
      left:16px;
      bottom:18px;
      /* FIX (Req 1 & 2): Đặt z-index cao cho nút bubble */
      z-index: 2147483645;
      pointer-events:none;
    }
    #motoai-bubble{
    ...
    #motoai-overlay{
      ...
      /* FIX (Req 1 & 2): Đặt z-index cao HƠN bubble để cửa sổ chat đè lên */
      z-index: 2147483646;
    }
    ...
    /* FIX (Req 3): Dùng biến CSS cho icon tools (thay vì var(--text)) */
    #motoai-header .tools button{
    ...
    /* FIX (Req 3): Dùng biến CSS */
    .m-msg.bot{
      background: var(--m10-bot-bg);
      color: var(--m10-bot-text);
    }
    ...
    #motoai-suggestions{
      ...
      flex-wrap:wrap;
      background: var(--m10-suggestion-bg);
      backdrop-filter:blur(8px);
    }
    #motoai-suggestions button{
      border:none;
      /* FIX (Req 3): Dùng biến CSS */
      background: var(--m10-suggestion-button-bg);
      color: var(--m10-suggestion-button-text);
    ...
    /* 🌙 Dark Mode (ĐÃ XÓA) */
    /* Toàn bộ logic @media (prefers-color-scheme: dark) và body.dark */
    /* đã được chuyển vào hàm JS setTheme_Adaptive để đồng bộ tuyệt đối */
  ...
  async function sendQuery(text){
    ...
    setTimeout(()=>{
      try{
        ...
        if(window.MotoAI_v10 && window.MotoAI_v10.smartAnswer){
  ...
  function init(){
    ...
    /* --- Refine+ Patch Logic --- */
    // 3. Auto dark sync: Đã được xử lý bởi hàm setTheme_Adaptive duy nhất
  ...
  window.MotoAI_v10 = Object.assign(window.MotoAI_v10 || {}, { // Sử dụng Object.assign để hợp nhất
    ...
    isSmart: (window.MotoAI_v10 && window.MotoAI_v10.isSmart) || false // Giữ trạng thái isSmart nếu đã được set
  });
...
// ⚙️ Tự động chọn theme (Dark / Light) - ĐÂY LÀ HÀM QUẢN LÝ THEME DUY NHẤT
(function(){ // <-- GỠ BỎ LỒNG NHAU NẾU CÓ
  const setTheme_Adaptive = ()=>{ // Đổi tên biến để tránh xung đột
    ...
    if(isDark){
      // --- CẤU HÌNH DARK MODE ---
      r.style.setProperty('--m10-card-bg','#0b0c0e'); // Card bg
      r.style.setProperty('--bg','#0f1113'); // Input bg
      r.style.setProperty('--text','#f2f2f7'); // Main text
      r.style.setProperty('--muted','#9a9a9f'); // Muted text (icons)
      r.style.setProperty('--footer-bg','rgba(25,25,30,0.9)'); // Footer bg
      r.style.setProperty('--glass-border','rgba(255,255,255,0.08)'); // Borders
      r.style.setProperty('--m10-bot-bg','rgba(35,37,39,0.9)'); // Bot message bg
      r.style.setProperty('--m10-bot-text','#f2f2f7'); // Bot message text
      r.style.setProperty('--m10-suggestion-bg','rgba(25,25,30,0.9)'); // Suggestions bar bg
      r.style.setProperty('--m10-suggestion-button-bg','rgba(0,122,255,0.2)'); // Suggestion btn bg
      r.style.setProperty('--m10-suggestion-button-text','var(--m10-accent)'); // Suggestion btn text
      document.body.dataset.theme='dark';
    }else{
      // --- CẤU HÌNH LIGHT MODE ---
      r.style.setProperty('--m10-card-bg','#ffffff');
      r.style.setProperty('--bg','#ffffff'); // Input bg
      r.style.setProperty('--text','#000000'); // Main text
      r.style.setProperty('--muted','#6e6e73'); // Muted text (icons)
      r.style.setProperty('--footer-bg','rgba(255,255,255,0.85)'); // Footer bg
      r.style.setProperty('--glass-border','rgba(0,0,0,0.08)'); // Borders
      r.style.setProperty('--m10-bot-bg','#f2f4f8'); // Bot message bg
      r.style.setProperty('--m10-bot-text','#000000'); // Bot message text
      r.style.setProperty('--m10-suggestion-bg','rgba(255,255,255,0.75)'); // Suggestions bar bg
      r.style.setProperty('--m10-suggestion-button-bg','rgba(0,122,255,0.1)'); // Suggestion btn bg
      r.style.setProperty('--m10-suggestion-button-text','var(--m10-accent)'); // Suggestion btn text
      document.body.dataset.theme='light';
    }
  };
  
  setTheme_Adaptive(); // Chạy ngay khi load
  // Lắng nghe thay đổi của hệ thống
  window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', setTheme_Adaptive);
  // Lắng nghe thay đổi class 'dark' trên body
  const mo = new MutationObserver(setTheme_Adaptive);
  mo.observe(document.body,{attributes:true,attributeFilter:['class']});
})(); // <-- GỠ BỎ LỒNG NHAU NẾU CÓ
...
(function(){ // <-- GỠ BỎ LỒNG NHAU NẾU CÓ
  ...
  if(!last || (now-last)>seventyTwoHrs){
    ...
    // FIX: Chờ MotoAI_v10.rebuildCorpus sẵn sàng
    const tryRebuild = () => {
      if(window.MotoAI_v10 && typeof window.MotoAI_v10.rebuildCorpus === 'function'){
        window.MotoAI_v10.rebuildCorpus();
        localStorage.setItem('MotoAI_lastCorpusBuild',now);
      } else {
        setTimeout(tryRebuild, 200); // Thử lại sau
      }
    };
    tryRebuild();
  }
...
// ✨ [ĐÃ XÓA] CSS Light Mode nâng cấp
// Lý do: Đã được hợp nhất vào hàm setTheme_Adaptive và khối CSS chính.
