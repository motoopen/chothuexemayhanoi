...
/* motoai_v38_2_clean_nodownmark_ui36.js
   ...
   ✅ KHỬ MARKDOWN: loại **, *, _, ~~, ``, #, >, [link](url), ![img](url), danh sách... trước khi hiển thị
   ✅ UI: input 36px, nút send 36px, màu chữ rõ ràng cả light/dark
   ✅ Public API: window.MotoAI_v38.{open,close,send,learnNow,getIndex,clearLearnCache}
*/
(function(){
  ...
  const DEF = {
    ...
    debug: false          // bật true nếu muốn xem console.debug/table
  };
  ...
  // KHỬ MARKDOWN TRIỆT ĐỂ (trả về plain text)
  function stripMarkdown(s){
    ...
    // links & images
    t = t.replace(/!$begin:math:display$([^$end:math:display$]*)\]$begin:math:text$[^)]+$end:math:text$/g, '$1');
    t = t.replace(/$begin:math:display$([^$end:math:display$]+)\]$begin:math:text$[^)]+$end:math:text$/g, '$1');
  ...
  #mta-card{position:fixed;right:16px;bottom:16px;width:min(420px,calc(100% - 24px));height:70vh;max-height:740px;background:var(--m-bg);color:var(--m-text);border-radius:18px;box-shadow:0 12px 40px rgba(0,0,0,.25);display:flex;flex-direction:column;overflow:hidden;transform:translateY(110%);transition:transform .22s cubic-bezier(.22,1,.36,1);will-change:transform}
  ...
  /* INPUT 36px + màu chữ rõ */
  #mta-input{background:#fff;border-top:1px solid rgba(0,0,0,.06);padding:6px 8px;display:flex;gap:8px;align-items:center}
  #mta-in{
    flex:1;border:1px solid rgba(0,0,0,.14);height:36px;padding:0 12px;border-radius:18px;
    background:#F2F4F7;color:#0b1220;font-size:14px;line-height:36px;box-sizing:border-box;-webkit-appearance:none;appearance:none
  }
  #mta-in::placeholder{color:#7a8599;opacity:.9}
  #mta-send{width:36px;height:36px;border:none;border-radius:50%;background:linear-gradient(160deg,#0084FF,#00B2FF);color:#fff;cursor:pointer;box-shadow:0 6px 18px rgba(0,132,255,.4);font-size:14px;display:flex;align-items:center;justify-content:center}
  ...
  @media(prefers-color-scheme:dark){
    ...
    #mta-in::placeholder{color:#9aa4b2}
  ...
  function addMsg(role,text){
    ...
    const clean = role==="bot" ? stripMarkdown(text) : text;   // KHỬ MARKDOWN khi hiển thị bot
    const el=document.createElement("div"); el.className="m-msg "+(role==="user"?"user":"bot"); el.textContent=clean;
    body.appendChild(el); body.scrollTop=body.scrollHeight;
    const arr=getSess(); arr.push({role,text:clean,t:Date.now()}); saveSess(arr);
  }
  function renderSess(){
  ...
  /* ====== AUTOLEARN: ưu tiên moto_sitemap.json, sau đó sitemap/crawl; HỌC NHIỀU SITE ====== */
  ...
  async function learnOneOrigin(origin){
    try{
      ...
      // 1) thử moto_sitemap.json ở gốc origin và thư mục hiện tại
  ...
  async function learnSites(origins, force){
    ...
    // Debug thống kê (tuỳ chọn)
    if(CFG.debug){
      const rows = Object.keys(results).map(k=>({domain:k, pages: (results[k]?.pages||[]).length, ts: results[k]?.ts}));
      console.log("[MotoAI] Learned domains:", rows);
  ...
  async function deepAnswer(userText){
    ...
    // Semantic retrieval + Extractive QA (KHỬ MARKDOWN trước khi trả)
    try{
      const top = searchIndex(q, 3);
      if(top && top.length){
        const t0 = top[0];
        if(CFG.smart.extractiveQA){
          const raw = (t0.title? (t0.title+'. ') : '') + (t0.text||'');
          const sn = bestSentences(raw, q, 2).join(' ');
          if(sn){
            const msg = `${stripMarkdown(sn)} — Xem thêm: ${t0.url}`;
            return naturalize(msg);
          }
        }
        const fallback = ((t0.title? (t0.title+' — ') : '') + (t0.text||'')).slice(0,180);
        return polite(`${stripMarkdown(fallback)} ... Xem thêm: ${t0.url}`);
      }
    }catch(e){}
  ...
  /* ====== TÍCH HỢP AUTO-PRICE LEARN (percentile) ====== */
  ...
  function bindEvents(){
    ...
    $("#mta-in").addEventListener("keydown", e=>{
      ...
      // Ẩn/hiện quick tags khi nhập liệu
  ...
  ready(async ()=>{
    ...
    if(!lastClean || (Date.now()-lastClean) > 7*24*3600*1000){
      ...
      console.log("MotoAI v38.2: maintenance OK");
    }
    ...
    // Gộp auto-prices học trước đó vào PRICE_TABLE
    mergeAutoPrices();
    ...
    if(CFG.autolearn){
      try{
        ...
        if(!last || (Date.now()-last) >= CFG.refreshHours*3600*1000){
          ...
          console.log("%cMotoAI v38.2 — learned from multiple sites","color:"+CFG.themeColor+";font-weight:bold;");
        }else{
          console.log("MotoAI v38.2 — skip learn (fresh cache)");
        }
      }catch(e){ console.warn("MotoAI v38.2 autolearn error", e); }
    }
  });
